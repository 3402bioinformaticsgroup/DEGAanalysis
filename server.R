    ...
    for (i in 1:np){
        for (j in 1:nr){     #abs(table[i,1]-table[i,2])/ (((table[i,3])^2 + (table[i,4])^2)/2)^0.5
            ...
            if(effective.size > 0.001){
                if (effective.size < 5){
            result <- pwr.t.test(n = NULL, d = effective.size,
...
output$plotDEG<-renderPlot({
    ...
    plot(log((ToPlot[,2]-ToPlot[,1])+0.1,2), ToPlot[,8], col=color,pch=16, main="Differentially Expressed Genes",
         xlab="LogMean", ylab="LogFC", ylim=c(-1*max(ToPlot[,8]), max(ToPlot[,8])))
